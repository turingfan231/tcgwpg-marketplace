      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between px-2.5 py-2">
          <div>
            ...
            <p className="mt-[2px] truncate text-[9.5px]" style={{ color: "#adadb4", fontWeight: 400 }}>
          ...
          <div className="mt-1.5 flex items-center border-t pt-1.5" style={{ borderColor: "rgba(255,255,255,0.035)" }}>
            ...
            <span className="truncate text-[9px]" style={{ color: "#bbbbc1", fontWeight: 500 }}>
              {sellerLabel(seller)}
            </span>
            {rating ? (
              <>
                <span aria-hidden="true" className="mx-1 text-[7px]" style={{ color: "#fbbf24" }}>★</span>
                <span className="text-[8px]" style={{ color: "#b6b6bc", fontWeight: 500 }}>
                  {rating.toFixed(1)}
                </span>
              </>
            ) : null}
            {listing.neighborhood ? (
              <>
                ...
                <MapPin aria-hidden="true" size={8} className="shrink-0" style={{ color: "#565661" }} />
                <span className="ml-[2px] truncate text-[8px]" style={{ color: "#b1b1b8", fontWeight: 400 }}>
                  {listing.neighborhood}
                </span>
              </>
            ) : null}
            <div className="flex-1" />
            <span className="mr-1 shrink-0 text-[8px]" style={{ color: "#a3a3aa", fontWeight: 400 }}>
              {compactTimeLabel(listing.sortTimestamp || listing.createdAt || listing.timeAgo)}
            </span>
          </div>
        </div>
      </button>
      <div className="flex w-[32px] shrink-0 items-center justify-center pr-2">
        <motion.button
        ...
        >
          <Heart
            ...
            style={{ color: favorite ? "#ef4444" : "#9a9aa2" }}
          />
        </motion.button>
      </div>
    </motion.article>
    ...
    >
      ...
      >
        <div className="relative aspect-[3/4] overflow-hidden">
          <img alt={listing.title} className="h-full w-full object-cover" decoding="async" loading="lazy" src={listingArtwork(listing)} />
          <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, transparent 40%, rgba(10,10,12,0.8) 100%)" }} />
          <div className="absolute left-1.5 top-1.5 rounded px-[5px] py-[1px]" style={{ background: "rgba(0,0,0,0.55)", border: `1px solid ${tone.color}20` }}>
            <span className="text-[7px]" style={{ color: tone.color, fontWeight: 700 }}>{tone.label}</span>
          </div>
          {Number(listing?.views || 0) > 50 ? (
        ...
        <div className="px-2 py-[5px]">
          <p className="truncate text-[10px]" style={{ color: "#e0e0e6", fontWeight: 600 }}>{listing.title}</p>
          <p className="mt-[1px] truncate text-[8px]" style={{ color: "#b1b1b8", fontWeight: 400 }}>{listing.setName || listing.set || listing.game}</p>
          <div className="mt-[3px] flex items-center gap-1">
            ...
            <span className="truncate text-[7.5px]" style={{ color: "#bbbbc1", fontWeight: 500 }}>{sellerLabel(listing?.seller || listing)}</span>
            <span className="ml-auto shrink-0 text-[6.5px]" style={{ color: "#a8a8af" }}>{compactTimeLabel(listing.sortTimestamp || listing.createdAt || listing.timeAgo)}</span>
          </div>
        </div>
      </motion.button>
      <motion.button
        ...
        onClick={() => onFavorite?.()}
      >
        <Heart fill={favorite ? "#ef4444" : "none"} size={11} strokeWidth={favorite ? 0 : 1.5} style={{ color: favorite ? "#ef4444" : "rgba(255,255,255,0.5)" }} />
      </motion.button>
    </motion.article>
  );
}
      ...
      >
        <div className="px-4 pb-2.5 pt-[max(0.85rem,env(safe-area-inset-top))] lg:px-6 lg:pb-5 lg:pt-8">
          ...
          <div className="mb-2 flex items-center gap-2 lg:mb-3 lg:max-w-none">
            ...
            >
              <Search size={14} style={{ color: "#85858f" }} className="shrink-0" />
              <input
                className="flex-1 bg-transparent text-[12.5px] outline-none placeholder:text-[#85858f]"
            ...
            >
              <SlidersHorizontal size={14} style={{ color: activeFilterCount > 0 ? "#f87171" : "#8c8c96" }} />
              {activeFilterCount > 0 ? (
          ...
          <div className="no-scrollbar flex gap-[5px] overflow-x-auto pb-2 lg:pb-3" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
            {gameCatalog.slice(0, 7).map((game) => {
              ...
              const icon = GAME_ICONS[game.slug] || "TCG";
              return (
                <motion.button
                  key={game.slug}
                ...
                >
                  ...
                  <span className="text-[11px]" style={{ color: active ? "#fca5a5" : "#c3c3ca", fontWeight: active ? 600 : 500 }}>
                    {gameLabel(game)}
                  </span>
                </motion.button>
              );
          ...
          <div className="flex items-center justify-between lg:max-w-none">
            ...
            <div className="flex overflow-hidden rounded-lg" style={{ border: "1px solid rgba(255,255,255,0.04)" }}>
              <button
                aria-label="Show listings as a list"
                className="px-2 py-[4px]"
                style={{ background: viewMode === "list" ? "rgba(239,68,68,0.08)" : "transparent" }}
                type="button"
                onClick={() => setViewMode("list")}
              >
                <LayoutList size={12} style={{ color: viewMode === "list" ? "#f87171" : "#8c8c96" }} />
              </button>
              <button
                aria-label="Show listings as a grid"
                className="px-2 py-[4px]"
                style={{ background: viewMode === "grid" ? "rgba(239,68,68,0.08)" : "transparent" }}
                type="button"
                onClick={() => setViewMode("grid")}
              >
                <Grid3X3 size={12} style={{ color: viewMode === "grid" ? "#f87171" : "#8c8c96" }} />
              </button>
            </div>
          </div>
        </div>
          ...
          <>
            ...
            >
              ...
              <div className="px-4 pb-6 pt-1">
                ...
                <p className="mb-2 text-[10px] uppercase tracking-[0.08em]" style={{ color: "#b9b9c1", fontWeight: 700 }}>
                  Condition
                </p>
