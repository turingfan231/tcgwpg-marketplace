...
app.get("/api/bootstrap", requireAllowedOrigin, publicApiRateLimit, async (req, res) => {
  if (!supabaseReadClient) {
  ...
  try {
    ...
    if (
      usePublicCache &&
      marketplaceBootstrapCache.payload &&
      marketplaceBootstrapCache.expiresAt > Date.now()
    ) {
      res.setHeader("Cache-Control", "public, max-age=15, stale-while-revalidate=45");
      return res.json(marketplaceBootstrapCache.payload);
    }
