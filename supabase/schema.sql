...
on public.collection_items for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "users manage own event preferences"
on public.user_event_preferences for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "event attendance is readable by everyone"
on public.user_event_preferences for select using (true);
